...
namespace JolDos2.Controllers
{
    public class HomeController : Controller
    {
        ...
        public IActionResult Index()
        {
            ...
            var result1 = User.IsInRole("admin");
            ...
            if (user != null)
            {
                ...
                if (!(_context.Books.Any(x => (x.TripId == tripId && x.BookedPassengerId == user.Id))))
                {
                    if (ModelState.IsValid)
                    {
                        ...
                        TempData["AlertMessage"] = "You  booked";
        ...
        {
            var booking = _context.Users.FirstOrDefault(b => b.Id == id);

            if (booking == null)
            {
                // If the user is not found, handle the case appropriately (e.g., display an error message)
                return NotFound();
            }

            // Block the user by setting the LockoutEnabled property to true and the LockoutEnd property to a future date
            booking.LockoutEnabled = true;
            booking.LockoutEnd = DateTimeOffset.MaxValue;
            var result = await _userManager.UpdateAsync(booking);
            if (result.Succeeded)
            {
                TempData["AlertMessage"] = "You blocked";
                TempData["AlertType"] = "success";
            ...
            {
                // Handle the case where blocking the user failed (e.g., display an error message)
                return View("Error");
            }
        ...
        public async Task<IActionResult> UnlockUser(string id)
        {
            ...
            // Block the user by setting the LockoutEnabled property to true and the LockoutEnd property to a future date
            booking.LockoutEnabled = false;
            booking.LockoutEnd = DateTimeOffset.MaxValue;
            var result = await _userManager.UpdateAsync(booking);
            if (result.Succeeded)
            {
                TempData["AlertMessage"] = "You unblocked";
                TempData["AlertType"] = "success";
                // User blocked successfully
                return RedirectToAction("IndexAdmin", "Home");
            }
